/**
 * features/personnel/hooks/usePersonnel.js
 *
 * Page orchestration hook for Personnel feature.
 */
...
} from '../services/personnelForm.service';
...
export function usePersonnel() {
  ...
  async function loadPageData() {
    setIsLoading(true);
    try {
      const [personnelData, stats] = await Promise.all([
        personnelApi.list(),
        stationsApi.list(),
      ]);
  ...
  async function submitPersonnelForm() {
    ...
    try {
      ...
      if (editingPersonnelId) {
        await personnelApi.update(editingPersonnelId, payload);
      } else {
  ...
  function askDelete(personnelRecord) {
    setPendingConfirmation({
      ...
      onConfirm: async () => {
        await personnelApi.delete(personnelRecord.id);
  ...
  function askToggleDutyStatus(personnelRecord) {
    ...
    setPendingConfirmation({
      ...
      onConfirm: async () => {
        await personnelApi.updateDutyStatus(personnelRecord.id, nextDutyStatus);
        setPendingConfirmation(null);
        await loadPageData();
      },
    });
